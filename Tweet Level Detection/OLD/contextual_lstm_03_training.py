"""
- We use the contextual_lstm_01 script to set up the data and _02 file to define the structure of the model
- We then train this

Need to:
- save vectorizer so that we don't need to keep rerunning this and can just load it instead (using pickle)

Notes:
- this is currently being trained on the dataset that has the Italian tweets removed (i.e. exclude social_spambots_3)
- also we've truncated the tokens down to the first 50
"""
...
train_m = df_trn.iloc[:second_split,1:7] # .astype(int).values
train_l = df_trn.iloc[:second_split,-1].values
test_s = trn_tweet_vector[second_split:,:]
test_m = df_trn.iloc[second_split:,1:7] # .astype(int).values
